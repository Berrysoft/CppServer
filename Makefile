ALL:= obj/ bin/ bin/server.out bin/file.so bin/cpu.so bin/version.so bin/disk.so bin/markdown.so bin/modules bin/style.css 
.PHONY: all
all: $(ALL)
.PHONY: clean
clean:
